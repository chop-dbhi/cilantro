define [
    ...
    nodeTemplate = _.template '
        <li class=node>{{ id }} {{ operator }} {{ value }}</li>
    '

    class DataContextTree extends Backbone.View
        _render: (model) ->
            if model.type is 'branch'
                $node = $ branchTemplate
                    type: model.get 'type'
                for child in model.get 'children'
                    $child = @_render child
                    if child.type is 'branch'
                        $node.append $child
                    else
                        $node.children('ul').append $child
            else
                $node = $ nodeTemplate model.toJSON()

            return $node

        render: ->
            @$el.html @_render @model
            return @


    { DataFields, DataConcepts, DataContexts, DataContextTree }
