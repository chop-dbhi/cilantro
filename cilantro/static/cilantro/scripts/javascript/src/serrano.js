...
define(['underscore', 'backbone'], function(_, Backbone) {
  var DataConcept, DataConcepts, DataContext, DataContextNode, DataContextTree, DataContexts, DataField, DataFields, branchTemplate, nodeTemplate;
  ...
  nodeTemplate = _.template('\
    ...
    ');
  DataContextTree = (function(_super) {

    __extends(DataContextTree, _super);

    function DataContextTree() {
      return DataContextTree.__super__.constructor.apply(this, arguments);
    }

    DataContextTree.prototype._render = function(model) {
      var $child, $node, child, _i, _len, _ref;
      if (model.type === 'branch') {
        $node = $(branchTemplate({
          type: model.get('type')
        }));
        _ref = model.get('children');
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          child = _ref[_i];
          $child = this._render(child);
    ...
    DataContextTree.prototype.render = function() {
      this.$el.html(this._render(this.model));
      return this;
    };
